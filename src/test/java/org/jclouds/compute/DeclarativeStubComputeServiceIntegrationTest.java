...
public class DeclarativeStubComputeServiceIntegrationTest extends
		BaseComputeServiceLiveTest {

	// private static final ExecResponse EXEC_GOOD = new ExecResponse("", "",
	// 0);
	// private static final ExecResponse EXEC_BAD = new ExecResponse("", "", 1);
	// private static final ExecResponse EXEC_RC_GOOD = new ExecResponse("0",
	// "",
	// 0);
	...
	@Override
	/* The SSH module is required for these IntegrationTests. */
	...
	@Override
	/*
	 * Check if hardware configurations match with spec ? Bigger has more cores,
	 * etc. Not really relevant for us at the moment...
	 */
	...
	// }
	//
	/* Copied from StubComputeServiceIntegrationTest */
	// protected void buildSocketTester() {
	// SocketOpen socketOpen = createMock(SocketOpen.class);
	//
	// // TODO Why do not really
	// //expect(socketOpen.apply(HostAndPort.fromParts("144.175.1.1", 22)))
	...
	//
	// @Override
	// public HostAndPort findOpenSocketOnNode(NodeMetadata node,
	// int port, long timeoutValue, TimeUnit timeUnits) {
	...
	// @Override
	// protected void configure() {
	// bind(AdminAccessConfiguration.class).toInstance(
	// new AdminAccessConfiguration() {
	// public Supplier<String> defaultAdminUsername() {
	// return Suppliers
	// .ofInstance("defaultAdminUsername");
	// }
	//
	// public Supplier<Map<String, String>> defaultAdminSshKeys() {
	// return Suppliers
	// .<Map<String, String>> ofInstance(ImmutableMap
	// .of("public",
	// "publicKey",
	// "private",
	...
	// expect(
	// factory.create(
	// HostAndPort.fromParts("144.175.1.1", 22),
	...
	// factory.create(
	// HostAndPort.fromParts("144.175.1.1", 22),
	// LoginCredentials.builder().user("web")
	// .privateKey(Pems.PRIVATE_PKCS1_MARKER)
	// .build())).andReturn(client1New).times(
	// 10);
	// runScriptAndService(client1, client1New);
	//
	// expect(
	// factory.create(
	// HostAndPort.fromParts("144.175.1.2", 22),
	// LoginCredentials.builder().user("root")
	// .password("password2").build()))
	// .andReturn(client2).times(4);
	// expect(
	// factory.create(
	// HostAndPort.fromParts("144.175.1.2", 22),
	// LoginCredentials.builder().user("root")
	// .password("password2").build()))
	// .andReturn(client2New);
	// expect(
	// factory.create(
	// HostAndPort.fromParts("144.175.1.2", 22),
	// LoginCredentials.builder().user("foo")
	// .privateKey(Pems.PRIVATE_PKCS1_MARKER)
	// .build())).andReturn(client2Foo);
	// expect(
	// factory.create(
	// HostAndPort.fromParts("144.175.1.2", 22),
	// LoginCredentials.builder().user("root")
	// .password("romeo").build())).andThrow(
	// new AuthorizationException("Auth fail", null));
	//
	// // run script without backgrounding (via predicate)
	// client2.connect();
	// expect(client2.exec("hostname\n")).andReturn(
	// new ExecResponse("stub-r\n", "", 0));
	// client2.disconnect();
	//
	...
	// client2,
	// "runScriptWithCreds",
	// Strings2.toStringAndClose(DeclarativeStubComputeServiceIntegrationTest.class
	...
	// "adminUpdate",
	// Strings2.toStringAndClose(DeclarativeStubComputeServiceIntegrationTest.class
	// .getResourceAsStream("/runscript_adminUpdate.sh")),
	// 2);
	// } catch (IOException e) {
	// Throwables.propagate(e);
	// }
	...
	// expect(client2Foo.getHostAddress()).andReturn("foo")
	// .atLeastOnce();
	// expect(client2Foo.exec("echo $USER\n")).andReturn(
	// new ExecResponse("foo\n", "", 0));
	// client2Foo.disconnect();
	//
	// expect(
	// factory.create(
	// HostAndPort.fromParts("144.175.1.3", 22),
	// LoginCredentials.builder().user("root")
	// .password("password3").build()))
	// .andReturn(client3).times(2);
	// expect(
	// factory.create(
	// HostAndPort.fromParts("144.175.1.4", 22),
	// LoginCredentials.builder().user("root")
	// .password("password4").build()))
	// .andReturn(client4).times(2);
	// expect(
	// factory.create(
	// HostAndPort.fromParts("144.175.1.5", 22),
	// LoginCredentials.builder().user("root")
	// .password("password5").build()))
	// .andReturn(client5).times(2);
	// expect(
	// factory.create(
	// HostAndPort.fromParts("144.175.1.6", 22),
	// LoginCredentials.builder().user("root")
	// .password("password6").build()))
	// .andReturn(client6).times(2);
	// expect(
	// factory.create(
	// HostAndPort.fromParts("144.175.1.7", 22),
	// LoginCredentials.builder().user("root")
	// .password("password7").build()))
	// .andReturn(client7).times(2);
	//
	...
	// eq(LoginCredentials.builder()
	// .user("defaultAdminUsername")
	// .privateKey(Pems.PRIVATE_PKCS1_MARKER)
	// .build()))).andReturn(client1);
	// expect(
	// factory.create(
	...
	// }
	//
	// private void runScriptAndService(SshClient client,
	// SshClient clientNew) {
	// client.connect();
	//
	// try {
	...
	// expect(client.exec("chmod 755 /tmp/init-" + scriptName))
	// .andReturn(EXEC_GOOD);
	// expect(
	// client.exec("ln -fs /tmp/init-" + scriptName + " "
	// + scriptName)).andReturn(EXEC_GOOD);
	// expect(client.getUsername()).andReturn("root")
	// .atLeastOnce();
	// expect(client.getHostAddress()).andReturn("localhost")
	// .atLeastOnce();
	// expect(client.exec("/tmp/init-" + scriptName + " init"))
	// .andReturn(EXEC_GOOD);
	// expect(client.exec("/tmp/init-" + scriptName + " start"))
	// .andReturn(EXEC_GOOD);
	// expect(client.exec("/tmp/init-" + scriptName + " status"))
	// .andReturn(EXEC_GOOD);
	// // next status says the script is done, since not found.
	// expect(client.exec("/tmp/init-" + scriptName + " status"))
	// .andReturn(EXEC_BAD);
	// expect(client.exec("/tmp/init-" + scriptName + " stdout"))
	// .andReturn(EXEC_GOOD);
	// expect(client.exec("/tmp/init-" + scriptName + " stderr"))
	// .andReturn(EXEC_GOOD);
	// expect(
	// client.exec("/tmp/init-" + scriptName
	// + " exitstatus")).andReturn(EXEC_RC_GOOD);
	//
	// // note we have to reconnect here, as we updated the login
	// // user.
	// client.disconnect();
	//
	// clientNew.connect();
	// expect(clientNew.getUsername()).andReturn("web")
	// .atLeastOnce();
	// expect(clientNew.getHostAddress()).andReturn("localhost")
	// .atLeastOnce();
	// expect(
	// clientNew
	// .exec("head -1 /usr/local/jetty/VERSION.txt | cut -f1 -d ' '\n"))
	// .andReturn(EXEC_GOOD);
	...
	// scriptName,
	// Strings2.toStringAndClose(DeclarativeStubComputeServiceIntegrationTest.class
	// .getResourceAsStream("/initscript_with_java.sh")),
	// nodeId);
	// } catch (IOException e) {
	// Throwables.propagate(e);
	// }
	...
	// }
	//
	// private void runScript(SshClient client, String scriptName,
	// String script, int nodeId) {
	...
	// expect(client.exec("/tmp/init-" + scriptName + " stderr"))
	// .andReturn(EXEC_GOOD);
	// expect(client.exec("/tmp/init-" + scriptName + " exitstatus"))
	// .andReturn(EXEC_RC_GOOD);
	// }
	//
	...
	// }
	// try {
	// String real = Strings2.toStringAndClose(((Payload) actual)
	// .openStream());
	...
	// }
	//
	// // TODO: I have absolutely no idea why I have to redeclare all this
	// cruft.
	// // If
	// // I don't, then we
